...
这实现了从可搜索对象中提取特征的 v2 版本，可搜索对象是对任何具有大小且可以在特定偏移量处读取的对象（例如文件或缓冲区）的抽象。
实现此方法是为了避免将整个文件加载到内存中或扫描整个缓冲区。
...
注意：此实现不支持提取 `mid` 特征，以及 `use_inputs_at_offsets`。
    ...
    def _get_beg_ints_with_padding(
        ...
        以一个（已剥离的）缓冲区作为输入，提取起始整数。
这将返回一个整数列表，其长度恰好等于 beg_size。如果
缓冲区大于所需长度，则仅提取起始部分。如果
缓冲区较短，则在末尾添加填充。
        """
    ...
    def _get_end_ints_with_padding(
        ...
        以一个（已剥离的）缓冲区作为输入，提取 end 个整数。这将返回一个长度恰好等于 end_size 的整数列表。
